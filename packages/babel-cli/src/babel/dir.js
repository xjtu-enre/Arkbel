...
export default async function ({ cliOptions, babelOptions, }) {
    async function write(src, base) {
        ...
        relative = util.withExtension(relative, cliOptions.keepFileExtension
    ...
    async function handleFile(src, base) {
        ...
        if ((cliOptions.copyFiles && written === FILE_TYPE.NON_COMPILABLE) ||
    ...
    if (cliOptions.watch) {
        ...
        watcher.onFilesChange(async (filenames) => {
            ...
            processing--;
            if (processing === 0 && !cliOptions.quiet)
                logSuccess();
        });
    }
}
