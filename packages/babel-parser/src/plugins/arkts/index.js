...
export var ArkTSParseContext;
(function (ArkTSParseContext) {
    ArkTSParseContext[ArkTSParseContext["TOP_FIRST"] = 0] = "TOP_FIRST";
    ArkTSParseContext[ArkTSParseContext["TOP"] = 1] = "TOP";
    ArkTSParseContext[ArkTSParseContext["CLOSURE"] = 2] = "CLOSURE";
})(ArkTSParseContext || (ArkTSParseContext = {}));
export default (superClass) => class ArkTSParserMixin extends superClass {
    ...
    maybeTakeDecorators(maybeDecorators, node, exportNode) {
        return super.maybeTakeDecorators(maybeDecorators, node, exportNode);
    }
    // ==================================
    // Overrides
    // ==================================
    ...
    parseStatementContent(flags, decorators) {
        if (this.isContextual(tt._struct)) {
            return this.arktsParseStruct(this.maybeTakeDecorators(decorators, 
    ...
    parseFunctionBody(node, allowExpression, isMethod = false) {
        if (this.inStylesFunction) {
            ...
            bodyNode.body = [];
            this.arktsParseExtendExpression(bodyNode, false, ArkTSParseContext.TOP_FIRST);
            this.scope.exit();
        ...
        if (this.inStructContext && this.inBuilderFunction) {
            ...
            // however for coherence with other function body parsing, we still parse it as an array.
            bodyNode.body = [];
            this.arktsParseBuildExpression(bodyNode, false, ArkTSParseContext.TOP_FIRST);
            this.scope.exit();
            this.expect(tt.braceR);
            node.body = this.finishNode(bodyNode, "BlockStatement");
            return;
        ...
        if (this.inStructContext && (this.inForEach || this.inLazyForeach)) {
            //foreach
            if (this.inForEach)
                this.inForEach = false; // to avoid 嵌套 Foreach
            else
                this.inLazyForeach = false;
            const bodyNode = this.startNode();
            this.expect(tt.braceL);
            this.scope.enter(ScopeFlag.FUNCTION || ScopeFlag.ARROW); //?
            // however for coherence with other function body parsing, we still parse it as an array.
            bodyNode.body = [];
            this.arktsParseForeachExpression(bodyNode, false, ArkTSParseContext.TOP_FIRST);
            this.scope.exit();
            this.expect(tt.braceR);
            node.body = this.finishNode(bodyNode, "BlockStatement");
            return;
     ...
     */
    // @ts-ignore(Babel 7 vs Babel 8) ArkTS:it occurs in ArkTSParserMixin in fact
    arktsParseBuildExpression(node, allowMultipleExpressions, context) {
        //console.log(node);
        if (node === undefined) {
            return node;
        }
            ...
            const MemberNode = this.arktsParseBuildExpression(thisNode, allowMultipleExpressions, context);
            const expNode = this.startNodeAtNode(MemberNode);
            ...
            const callNode = this.startNode();
            callNode.callee = this.parseIdentifier();
            if (callNode.callee.name == "ForEach") {
                this.inForEach = true;
            }
            if (callNode.callee.name == "LazyForEach")
                this.inLazyForeach = true;
            this.expect(tt.parenL);
            // @ts-ignore(Babel 7 vs Babel 8) ArkTS:it occurs in ArkTSParserMixin in fact
            callNode.arguments = this.parseCallExpressionArguments(tt.parenR);
            // Trailing closure
            const closureNode = this.startNode();
            closureNode.body = [];
            if (this.match(tt.braceL)) {
                callNode.trailingClosure = this.arktsParseBuildExpression(closureNode, true, ArkTSParseContext.CLOSURE);
            }
            ...
            const expNode = this.startNodeAtNode(callNode);
            const maybeNode = this.arktsParseBuildExpression(callNode, allowMultipleExpressions, context);
            this.finishNode(expNode, "ExpressionStatement");
            ...
            const memberNode = this.startNodeAtNode(node);
            ...
            return this.arktsParseBuildExpression(memberNode, allowMultipleExpressions, context);
            ...
            const callNode = this.startNodeAtNode(node);
            ...
            const ifNode = this.startNode();
            this.next(); // eat `if`
            // @ts-ignore(Babel 7 vs Babel 8) ArkTS:it occurs in ArkTSParserMixin in fact
            ifNode.test = this.parseHeaderExpression();
            const consequentNode = this.startNode();
            consequentNode.body = [];
            if (this.match(tt.braceL)) {
                ifNode.consequent = this.arktsParseBuildExpression(consequentNode, true, ArkTSParseContext.CLOSURE);
            }
            else {
                this.unexpected(null, tt.braceL);
            }
            ifNode.alternate = this.eat(tt._else)
                ? this.arktsParseBuildExpression(null, true, ArkTSParseContext.CLOSURE)
                : null;
            ...
            let blockNode;
            if (node === null) {
                blockNode = this.startNode();
                blockNode.body = [];
            }
            this.next(); // eat `{`
            this.scope.enter(ScopeFlag.FUNCTION);
            this.arktsParseBuildExpression(node ?? blockNode, true, ArkTSParseContext.CLOSURE);
            this.scope.exit();
            this.expect(tt.braceR);
            if (node === null) {
                return blockNode;
    ...
    // @ts-ignore(Babel 7 vs Babel 8) ArkTS:it occurs in ArkTSParserMixin in fact
    arktsParseExtendExpression(node, allowMultipleExpressions, context) {
        if (node === undefined) {
            return node;
        }
        else if (this.match(tt._this)) {
            ...
            this.next();
            this.finishNode(thisNode, "ThisExpression");
            const MemberNode = this.arktsParseExtendExpression(thisNode, allowMultipleExpressions, context);
            const expNode = this.startNodeAtNode(MemberNode);
            this.finishNode(expNode, "ExpressionStatement");
            expNode.expression = MemberNode;
            node.body.push(expNode);
            ...
            const calleenode = this.startNode();
            ...
            // but for extensibility reason, the rejection is done by previous if statement
            const expNode = this.startNodeAtNode(callNode);
            const maybeNode = this.arktsParseExtendExpression(callNode, allowMultipleExpressions, context);
            this.finishNode(expNode, "ExpressionStatement");
            expNode.expression = maybeNode ?? callNode;
            node.body.push(expNode);
            // Pass BlockStatement node in so that new nodes can be pushed into
            ...
            memberNode.property = this.parseIdentifier();
            this.finishNode(memberNode, "MemberExpression");
            return this.arktsParseExtendExpression(memberNode, allowMultipleExpressions, context);
        }
        else if (this.eat(tt.parenL)) {
            const callNode = this.startNodeAtNode(node);
            callNode.callee = node;
    ...
    // @ts-ignore(Babel 7 vs Babel 8) ArkTS:it occurs in ArkTSParserMixin in fact
    arktsParseForeachExpression(node, allowMultipleExpressions, context) {
        if (node === undefined) {
            return node;
        }
        else if (this.match(tt._this)) {
            ...
            this.next();
            this.finishNode(thisNode, "ThisExpression");
            const MemberNode = this.arktsParseForeachExpression(thisNode, allowMultipleExpressions, context);
            const expNode = this.startNodeAtNode(MemberNode);
            this.finishNode(expNode, "ExpressionStatement");
            expNode.expression = MemberNode;
            node.body.push(expNode);
            ...
            closureNode.body = [];
            if (this.match(tt.braceL)) {
                callNode.trailingClosure = this.arktsParseForeachExpression(closureNode, true, ArkTSParseContext.CLOSURE);
            }
            this.finishNode(callNode, "ArkTSCallExpression");
            // Below creates an ExpressionStatement that wraps CallExpression
            // This does not reject multiple calls at top level, which is invalid in ArkTS
            // but for extensibility reason, the rejection is done by previous if statement
            const expNode = this.startNodeAtNode(callNode);
            const maybeNode = this.arktsParseForeachExpression(callNode, allowMultipleExpressions, context);
            this.finishNode(expNode, "ExpressionStatement");
            expNode.expression = maybeNode ?? callNode;
            node.body.push(expNode);
            // Pass BlockStatement node in so that new nodes can be pushed into
            ...
            this.finishNode(memberNode, "MemberExpression");
            // MemberExpression is for sure to be appended with CallExpression, or syntax error
            return this.arktsParseForeachExpression(memberNode, allowMultipleExpressions, context);
        }
        else if (this.eat(tt.parenL)) {
            const callNode = this.startNodeAtNode(node);
            callNode.callee = node;
            ...
            consequentNode.body = [];
            if (this.match(tt.braceL)) {
                ifNode.consequent = this.arktsParseForeachExpression(consequentNode, true, ArkTSParseContext.CLOSURE);
            }
            else {
                this.unexpected(null, tt.braceL);
            }
            ifNode.alternate = this.eat(tt._else)
                ? this.arktsParseForeachExpression(null, true, ArkTSParseContext.CLOSURE)
                : null;
            node.body.push(this.finishNode(ifNode, "IfStatement"));
        }
        else if (this.match(tt.braceL)) {
            ...
            this.next(); // eat `{`
            this.scope.enter(ScopeFlag.FUNCTION);
            this.arktsParseForeachExpression(node ?? blockNode, true, ArkTSParseContext.CLOSURE);
            this.scope.exit();
            this.expect(tt.braceR);
            if (node === null) {
                return blockNode;
