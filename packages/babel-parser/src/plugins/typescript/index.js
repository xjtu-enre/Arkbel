...
import { tokenIsIdentifier, tokenIsTSDeclarationStart, tokenIsTSTypeOperator, tokenOperatorPrecedence, tokenIsKeywordOrIdentifier, tt, tokenIsTemplate, tokenCanStartExpression, } from "../../tokenizer/types.ts";
...
export default (superClass) => class TypeScriptParserMixin extends superClass {
    getScopeHandler() {
    ...
    tsParseBindingListForSignature() {
        const list = super.parseBindingList(tt.parenR, charCodes.rightParenthesis, ParseBindingListFlags.IS_FUNCTION_PARAMS);
    ...
    tsParseConstraintForInferType() {
        if (this.eat(tt._extends)) {
            ...
            if (this.state.inDisallowConditionalTypesContext ||
    ...
    tsSkipParameterStart() {
        ...
        if (this.match(tt.bracketL)) {
            ...
            const previousErrorCount = errors.length;
            try {
                super.parseBindingList(tt.bracketR, charCodes.rightSquareBracket, ParseBindingListFlags.ALLOW_EMPTY);
                return errors.length === previousErrorCount;
            }
            catch {
                return false;
    ...
    tsParseTypeAliasDeclaration(node) {
        ...
        node.typeAnnotation = this.tsInType(() => {
            ...
            this.expect(tt.eq);
            if (this.isContextual(tt._intrinsic) &&
    ...
    tsIsExternalModuleReference() {
        return (this.isContextual(tt._require) &&
    ...
    tsTryParseDeclare(nany) {
        ...
        return this.tsInAmbientContext(() => {
            switch (startType) {
                case tt._function:
                    nany.declare = true;
                    return super.parseFunctionStatement(nany, 
                    /* async */ false, 
                ...
                default:
                    if (tokenIsIdentifier(startType)) {
                        return this.tsParseDeclaration(nany, this.state.value, 
                        /* next */ true, 
                        /* decorators */ null);
                    }
            }
    ...
    parseAssignableListItem(flags, decorators) {
        ...
        if (!(flags & ParseBindingListFlags.IS_CONSTRUCTOR_PARAMS) &&
    ...
    parseExprOp(left, leftStartLoc, minPrec) {
        let isSatisfies;
        if (tokenOperatorPrecedence(tt._in) > minPrec &&
            !this.hasPrecedingLineBreak() &&
    ...
    */
    ...
    parseExport(node, decorators) {
        if (this.match(tt._import)) {
            ...
            if (this.isContextual(tt._type) &&
                ...
                maybeDefaultIdentifier = this.parseMaybeImportPhase(node, 
                /* isExport */ false);
            }
            else {
                node.importKind = "value";
    ...
    tsIsStartOfStaticBlocks() {
        return (this.isContextual(tt._static) &&
    ...
    bindingType) {
        ...
        super.parseClassId(node, isStatement, optionalId, node.declare
    ...
    tsParseAbstractDeclaration(node, decorators) {
        if (this.match(tt._class)) {
            node.abstract = true;
            return this.maybeTakeDecorators(decorators, this.parseClass(node, 
            /* isStatement */ true, 
            /* optionalId */ false));
        }
    ...
    bindingType) {
        if (!importedIsString && isMaybeTypeOnly) {
            this.parseTypeOnlyImportExportSpecifier(specifier, 
            /* isImport */ true, isInTypeOnlyImport);
            return this.finishNode(specifier, "ImportSpecifier");
        }
        specifier.importKind = "value";
        return super.parseImportSpecifier(specifier, importedIsString, isInTypeOnlyImport, isMaybeTypeOnly, isInTypeOnlyImport
    ...
    parseTypeOnlyImportExportSpecifier(node, isImport, isInTypeOnlyImportExport) {
        ...
        if (hasTypeSpecifier && isInTypeOnlyImportExport) {
            this.raise(isImport
                ...
                : TSErrors.TypeModifierIsUsedInTypeExports, loc);
        }
        ...
        if (isImport) {
            this.checkIdentifier(node[rightOfAsKey], hasTypeSpecifier
                ? BindingFlag.TYPE_TS_TYPE_IMPORT
                : BindingFlag.TYPE_TS_VALUE_IMPORT);
        }
    }
