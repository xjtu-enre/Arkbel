...
import { Position, SourceLocation, createPositionWithColumnOffset, } from "../util/location.ts";
import CommentsParser from "../parser/comments.ts";
import * as charCodes from "charcodes";
import { isIdentifierStart, isIdentifierChar } from "../util/identifier.ts";
import { tokenIsKeyword, tokenLabelName, tt, keywords as keywordTypes, } from "./types.ts";
import { Errors, } from "../parse-error.ts";
import { lineBreakG, isNewLine, isWhitespace, skipWhiteSpace, skipWhiteSpaceInLine, } from "../util/whitespace.ts";
...
export default class Tokenizer extends CommentsParser {
            ...
            switch (ch) {
                ...
                default:
                    if (isWhitespace(ch)) {
                        ...
                        if (this.input.charCodeAt(pos + 1) === charCodes.dash &&
                        ...
                        this.options.annexB) {
                        const pos = this.state.pos;
                        if (this.input.charCodeAt(pos + 1) === charCodes.exclamationMark &&
    ...
    readToken_numberSign() {
        ...
        if (next === charCodes.leftCurlyBrace ||
            ...
            if (this.getPluginOption("recordAndTuple", "syntaxType") === "bar") {
                throw this.raise(next === charCodes.leftCurlyBrace
                    ? Errors.RecordExpressionHashIncorrectStartSyntaxType
                    : Errors.TupleExpressionHashIncorrectStartSyntaxType, this.state.curPosition());
            }
            this.state.pos += 2;
            if (next === charCodes.leftCurlyBrace) {
                // #{
    ...
    readToken_eq_excl(code) {
        ...
        const next = this.input.charCodeAt(this.state.pos + 1);
        if (next === charCodes.equalsTo) {
            this.finishOp(tt.equality, this.input.charCodeAt(this.state.pos + 2) === charCodes.equalsTo
                ? 3
                : 2);
            return;
        }
    ...
    readNumber(startsWithDot) {
        ...
        const hasLeadingZero = this.state.pos - start >= 2 &&
    ...
    readString(quote) {
        const { str, pos, curLine, lineStart } = readStringContents(quote === charCodes.quotationMark ? "double" : "single", this.input, this.state.pos + 1, // skip the quote
    ...
    readTemplateToken() {
        ...
        const { str, firstInvalidLoc, pos, curLine, lineStart } = readStringContents("template", this.input, this.state.pos + 1, // skip '`' or `}`
    ...
    errorHandlers_readStringContents_string = {
        ...
        unterminated: (pos, lineStart, curLine) => {
            throw this.raise(Errors.UnterminatedString, // Report the error at the string quote
