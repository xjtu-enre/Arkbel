...
export default declare(api => {
    ...
    const nodeSupportsIMR = !emptyNodeTarget &&
        !isRequired("node", { node: nodeTarget }, imrCompatData);
    const webSupportsIMR = !emptyWebTargets && !isRequired("web", webTargets, imrCompatData);
    let helperESM;
    let helperCJS;
    const getHelper = (file) => {
        ...
        throw new Error(`@babel/plugin-proposal-import-wasm-source can only be used when not ` +
        ...
        visitor: {
            Program(path) {
                ...
                for (const decl of path.get("body")) {
                    if (!decl.isImportDeclaration({ phase: "source" }))
                        continue;
    ...
    function buildHelper(toCommonJS) {
        ...
        const p = ({ web: w, node: n, webIMR: wI = webSupportsIMR, nodeIMR: nI = nodeSupportsIMR, toCJS: c = toCommonJS, }) => +w + (+n << 1) + (+wI << 2) + (+nI << 3) + (+c << 4);
