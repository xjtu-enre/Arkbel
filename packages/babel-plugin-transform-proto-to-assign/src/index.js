...
export default declare(api => {
    ...
    function buildDefaultsCallExpression(expr, ref, file) {
        return t.expressionStatement(t.callExpression(file.addHelper("defaults"), [
    ...
    return {
        ...
        visitor: {
            AssignmentExpression(path, { file }) {
                if (!isProtoAssignmentExpression(path.node.left))
                    return;
                const nodes = [];
                const left = path.node.left.object;
                const temp = path.scope.maybeGenerateMemoised(left);
                if (temp) {
                    nodes.push(t.expressionStatement(t.assignmentExpression("=", temp, 
