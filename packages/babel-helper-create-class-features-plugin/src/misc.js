...
export function injectInitialization(path, constructor, nodes, renamer, lastReturnsThis) {
    if (!nodes.length)
        return;
    ...
    if (!constructor) {
        ...
        [constructor] = path
            .get("body")
            .unshiftContainer("body", newConstructor);
    }
    if (renamer) {
    ...
    if (isDerived) {
        ...
        for (const bareSuper of bareSupers) {
            ...
            if (!bareSuper.parentPath.isExpressionStatement()) {
                ...
                if (!lastReturnsThis)
                    allNodes.push(t.thisExpression());
...
export function extractComputedKeys(path, computedPaths, file) {
    ...
    for (const computedPath of computedPaths) {
        ...
        if (!computedKey.isConstantExpression()) {
            ...
            const isUidReference = t.isIdentifier(computedKey.node) && scope.hasUid(computedKey.node.name);
            const isMemoiseAssignment = computedKey.isAssignmentExpression({ operator: "=" }) &&
            ...
            if (isUidReference) {
                ...
                computedNode.key = t.cloneNode(computedNode.key.left);
            }
            else {
