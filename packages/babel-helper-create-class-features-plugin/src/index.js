...
import { buildPrivateNamesNodes, buildPrivateNamesMap, transformPrivateNamesUsage, buildFieldsInitNodes, buildCheckInRHS, } from "./fields.ts";
...
import { enableFeature, FEATURES, isLoose, shouldTransform, } from "./features.ts";
...
export function createClassFeaturePlugin({ name, feature, loose, manipulateOptions, api, inherits, decoratorVersion, }) {
    if (feature & FEATURES.decorators) {
        if (process.env.BABEL_8_BREAKING) {
            return createDecoratorTransform(api, { loose }, "2023-05", inherits);
        }
        else {
            if (decoratorVersion === "2021-12" ||
    ...
    if (privateFieldsAsProperties && privateFieldsAsSymbols) {
        throw new Error(`Cannot enable both the "privateFieldsAsProperties" and ` +
    ...
    if (loose === true) {
        ...
        if (explicit.length !== 0) {
            console.warn(`[${name}]: You are using the "loose: true" option and you are` +
    ...
    return {
        ...
        visitor: {
            Class(path, { file }) {
                if (file.get(versionKey) !== PACKAGE_JSON.version)
                    return;
                if (!shouldTransform(path, file))
                    return;
                const pathIsClassDeclaration = path.isClassDeclaration();
                if (pathIsClassDeclaration)
                    assertFieldTransformed(path);
                ...
                if (instanceNodes.length > 0) {
                    injectInitialization(path, constructor, instanceNodes, (referenceVisitor, state) => {
                        ...
                        for (const prop of props) {
                            ...
                            if (t.isStaticBlock?.(prop.node) || prop.node.static)
                                continue;
