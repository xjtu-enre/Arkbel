...
export default declare(api => {
    ...
    function allReferencesAreProps(scope, node) {
        ...
        return !!binding?.referencePaths.every(path => path.parentPath.isMemberExpression({ object: path.node }));
    }
    return {
            ...
            defineCommonJSHook(file, {
                ...
                getWrapperPayload(source, metadata, importNodes) {
                    ...
                    for (const node of importNodes) {
                        ...
                        if (node.phase !== "defer")
                            return null;
                        if (!allReferencesAreProps(file.scope, node))
                            needsProxy = true;
                    }
        ...
        visitor: {
            Program(path) {
                if (this.file.get("@babel/plugin-transform-modules-*") !== "commonjs") {
                    throw new Error(`@babel/plugin-proposal-import-defer can only be used when` +
                ...
                for (const child of path.get("body")) {
                    if ((child.isImportDeclaration() && child.node.phase == null) ||
