...
export default function transformClass(path, file, builtinClasses, isLoose, assumptions, supportUnicodeId) {
    ...
    function maybeCreateConstructor() {
      ...
      `;
        ...
        classBodyPath.unshiftContainer("body", t.classMethod("constructor", t.identifier("constructor"), params, body));
    }
    function buildBody() {
    ...
    function wrapSuperCall(bareSuper, superRef, thisRef, body) {
        ...
        if (assumptions.superIsCallableConstructor) {
            ...
            if (bareSuperNode.arguments?.length) {
                 ...
                 * super(...iter);
                 */
                if (bareSuperNodeArguments.length === 1 &&
    ...
    function verifyConstructor() {
        ...
        for (const thisPath of classState.superThises) {
            ...
            thisPath.replaceWith(t.callExpression(classState.file.addHelper("assertThisInitialized"), [
                thisRef(),
            ]));
        }
        const bareSupers = [];
        ...
        if (t.isStringLiteral(key)) {
            ...
            if (node.kind === "method") {
                ...
                fn =
                    ...
                    // @ts-expect-error Fixme: we are passing a ClassMethod to nameFunction, but nameFunction
                    // does not seem to support it
                    { id: key, node: node, scope }, undefined, supportUnicodeId) ?? fn;
            }
        }
        else {
    ...
    function pushConstructorToBody() {
        if (classState.pushedConstructor)
            return;
    ...
    function pushInheritsToBody() {
        if (!classState.isDerived || classState.pushedInherits)
            return;
    ...
    function extractDynamicKeys() {
        ...
        for (const elem of node.body.body) {
            if (!t.isClassMethod(elem) || !elem.computed)
                continue;
            if (scope.isPure(elem.key, /* constants only*/ true))
                continue;
    ...
    function classTransformer(path, file, builtinClasses, isLoose) {
        ...
        setState({
            extendsNative: t.isIdentifier(classState.superName) &&
                builtinClasses.has(classState.superName.name) &&
                !classState.scope.hasBinding(classState.superName.name, 
                /* noGlobals */ true),
        });
        ...
        if (!assumptions.noClassCalls) {
            constructorBody.body.unshift(t.expressionStatement(t.callExpression(classState.file.addHelper("classCallCheck"), [
                t.thisExpression(),
                t.cloneNode(classState.classRef),
            ])));
        }
