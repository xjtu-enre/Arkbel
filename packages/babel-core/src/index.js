if (!process.env.IS_PUBLISH && !USE_ESM && process.env.BABEL_8_BREAKING) {
    throw new Error("BABEL_8_BREAKING is only supported in ESM. Please run `make use-esm`.");
}
...
export { createConfigItem, createConfigItemSync, createConfigItemAsync, } from "./config/index.ts";
export { loadPartialConfig, loadPartialConfigSync, loadPartialConfigAsync, loadOptions, loadOptionsAsync, } from "./config/index.ts";
import { loadOptionsSync } from "./config/index.ts";
export { loadOptionsSync };
export { transform, transformSync, transformAsync, } from "./transform.ts";
export { transformFile, transformFileSync, transformFileAsync, } from "./transform-file.ts";
export { transformFromAst, transformFromAstSync, transformFromAstAsync, } from "./transform-ast.ts";
