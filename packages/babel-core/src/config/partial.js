...
import { findConfigUpwards, resolveShowConfigPath, ROOT_CONFIG_FILENAMES, } from "./files/index.ts";
...
function resolveRootMode(rootDir, rootMode) {
    switch (rootMode) {
        ...
        case "upward": {
            ...
            throw Object.assign(new Error(`Babel was run with rootMode:"upward" but a root could not ` +
...
export default function* loadPrivatePartialConfig(inputOpts) {
    ...
    const { envName = getEnv(), cwd = ".", root: rootDir = ".", rootMode = "root", caller, cloneInputAst = true, } = args;
...
export function* loadPartialConfig(opts) {
    ...
    (options.plugins || []).forEach(item => {
        ...
        if (item.value instanceof Plugin) {
            throw new Error("Passing cached plugin instances is not supported in " +
