...
function* readConfigCode(filepath, data) {
    if (!nodeFs.existsSync(filepath))
        return null;
    let options = yield* loadCodeDefault(filepath, "You appear to be using a native ECMAScript module configuration " +
    ...
    if (typeof options.then === "function") {
        ...
        // but not a promise
        options.catch?.(() => { });
        throw new ConfigError(`You appear to be using an async configuration, ` +
            `which your current version of Babel does not support. ` +
            `We may add support for this in the future, ` +
            `but if you're on the most recent version of @babel/core and still ` +
            `seeing this error, then you'll need to synchronously return your config.`, filepath);
    }
...
const readConfigJSON5 = makeStaticFileCache((filepath, content) => {
    ...
    if (!options)
        throw new ConfigError(`No config detected`, filepath);
...
export function* findRelativeConfig(packageData, envName, caller) {
    ...
    for (const loc of packageData.directories) {
        if (!config) {
            config = yield* loadOneConfig(RELATIVE_CONFIG_FILENAMES, loc, envName, caller, packageData.pkg?.dirname === loc
