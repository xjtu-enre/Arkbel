...
function* requireModule(type, name) {
    if (!process.env.BABEL_8_BREAKING) {
        if (!(yield* isAsync()) && LOADING_MODULES.has(name)) {
            throw new Error(`Reentrant ${type} detected trying to load "${name}". This module is not ignored ` +
    ...
    try {
        ...
        if (process.env.BABEL_8_BREAKING) {
            return yield* loadCodeDefault(name, `You appear to be using a native ECMAScript module ${type}, ` +
