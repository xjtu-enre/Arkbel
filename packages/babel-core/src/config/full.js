...
import { forwardAsync, maybeAsync, isThenable, } from "../gensync-utils/async.ts";
...
import { validate, checkNoUnwrappedItemOptionPairs, } from "./validation/options.ts";
...
const makeDescriptorLoader = (apiFactory) => makeWeakCache(function* ({ value, options, dirname, alias }, cache) {
    ...
    if (isThenable(item)) {
        ...
        yield* [];
        throw new Error(`You appear to be using a promise as a plugin, ` +
    ...
    if (externalDependencies.length > 0 &&
        ...
        let error = `A plugin/preset has external untracked dependencies ` +
