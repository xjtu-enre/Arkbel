...
function hasProto(node) {
    return node.properties.some(value => t.isObjectProperty(value, { computed: false, shorthand: false }) &&
    ...
    ["@babel/plugin-transform-object-rest-spread", { "loose": true, "useBuiltIns": ${useBuiltInsFormatted} }]
            ...
            if (RUNTIME_DEFAULT === "classic") {
                ...
                if (useSpread && useBuiltIns) {
                    throw new Error("transform-react-jsx currently only accepts useBuiltIns or useSpread " +
                        "but not both");
                }
            }
        }
...
You can set \`throwIfNamespace: false\` to bypass this warning.`);
        ...
        function accumulateAttribute(array, attribute) {
            ...
            if (t.isJSXNamespacedName(attribute.node.name)) {
                ...
                attribute.node.name = t.stringLiteral(attribute.node.name.namespace.name +
                    ":" +
            ...
            array.push(t.inherits(t.objectProperty(
        ...
        function buildJSXFragmentCall(path, file) {
            ...
            const children = t.react.buildChildren(path.node);
            args.push(t.objectExpression(children.length > 0
                ? [
        ...
        function buildCreateElementOpeningElementAttributes(file, path, attribs) {
            ...
            for (const attr of attribs) {
                const { node } = attr;
                const name = t.isJSXAttribute(node) &&
