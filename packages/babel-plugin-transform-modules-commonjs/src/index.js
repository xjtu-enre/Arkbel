...
export default declare((api, options) => {
    ...
    strictNamespace = false, mjsStrictNamespace = strictNamespace, allowTopLevelThis, strict, strictMode, noInterop, importInterop, lazy = false, 
    // Defaulting to 'true' for now. May change before 7.x major.
    allowCommonJSExports = true, loose = false, } = options;
  ...
  `;
        ...
        AssignmentExpression(path) {
            ...
            if (left.isIdentifier()) {
                ...
                const localName = Object.keys(ids).filter(localName => {
                    if (localName !== "module" && localName !== "exports")
                        return false;
                    return (this.scope.getBinding(localName) ===
    ...
    return {
        ...
        pre() {
            ...
            if (lazy)
                defineCommonJSHook(this.file, lazyImportsHook(lazy));
        },
        visitor: {
            ["CallExpression" +
                ...
                if (!this.file.has("@babel/plugin-proposal-dynamic-import"))
                    return;
                if (path.isCallExpression() && !t.isImport(path.node.callee))
                    return;
                let { scope } = path;
                do {
            ...
            Program: {
              ...
              `;
                    ...
                    path.get("body").forEach(path => {
                        if (headers.indexOf(path.node) === -1)
                            return;
