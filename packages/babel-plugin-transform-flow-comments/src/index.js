...
export default declare(api => {
    ...
    function attachComment({ ofPath, toPath, where = "trailing", optional = false, comments = generateComment(ofPath, optional), keepType = false, }) {
        if (!toPath?.node) {
    ...
    return {
        ...
        visitor: {
            ...
            ImportDeclaration(path) {
                ...
                const typeSpecifiers = node.specifiers.filter(specifier => specifier.type === "ImportSpecifier" &&
                    isTypeImport(specifier.importKind));
                const nonTypeSpecifiers = node.specifiers.filter(specifier => specifier.type !== "ImportSpecifier" ||
                    !isTypeImport(specifier.importKind));
