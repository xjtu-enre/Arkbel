...
var PROP_KIND;
(function (PROP_KIND) {
    PROP_KIND[PROP_KIND["FIELD"] = 0] = "FIELD";
    PROP_KIND[PROP_KIND["ACCESSOR"] = 1] = "ACCESSOR";
    PROP_KIND[PROP_KIND["METHOD"] = 2] = "METHOD";
    PROP_KIND[PROP_KIND["GETTER"] = 3] = "GETTER";
    PROP_KIND[PROP_KIND["SETTER"] = 4] = "SETTER";
    PROP_KIND[PROP_KIND["CLASS"] = 5] = "CLASS";
    PROP_KIND[PROP_KIND["STATIC"] = 8] = "STATIC";
    PROP_KIND[PROP_KIND["DECORATORS_HAVE_THIS"] = 16] = "DECORATORS_HAVE_THIS";
})(PROP_KIND || (PROP_KIND = {}));
/**
  Basic usage:
 ...
 */
export default /* @no-mangle */ function applyDecs2305(targetClass, memberDecs, classDecs, classDecsHaveThis, instanceBrand, parentClass) {
    ...
    function applyDec(Class, decInfo, decoratorsHaveThis, name, kind, metadata, initializers, ret, isStatic, isPrivate, isField, isAccessor, hasPrivateBrand) {
        ...
        if (!isClass) {
            ...
            var desc = {}, init = [], key = kind === PROP_KIND.GETTER
                ? "get"
        ...
        if (!isField && !isClass) {
            if (isPrivate) {
                if (isAccessor) {
                    ...
                    ret.push(kind === PROP_KIND.METHOD
                        ? desc[key]
    ...
    function applyMemberDecs(Class, decInfos, instanceBrand, metadata) {
        ...
        for (var i = 0; i < decInfos.length; i++) {
            ...
            if (!isField && !isPrivate) {
                ...
                if (existingKind === true ||
                    (existingKind === PROP_KIND.GETTER && kind !== PROP_KIND.SETTER) ||
                    (existingKind === PROP_KIND.SETTER && kind !== PROP_KIND.GETTER)) {
                    throw new Error("Attempted to decorate a public method/accessor that has the same name as a previously decorated public method/accessor. This is not currently supported by the decorators plugin. Property name was: " +
                        name);
                }
                existingNonFields.set(key, kind > PROP_KIND.METHOD ? kind : true);
            }
            applyDec(isStatic ? Class : Class.prototype, decInfo, decoratorsHaveThis, isPrivate ? "#" + name : toPropertyKey(name), kind, metadata, isStatic
                ? (staticInitializers = staticInitializers || [])
                : (protoInitializers = protoInitializers || []), ret, isStatic, isPrivate, isField, kind === PROP_KIND.ACCESSOR, isStatic && isPrivate ? staticBrand : instanceBrand);
        }
    ...
    return {
        ...
        get c() {
            ...
            var initializers = [];
            return (classDecs.length && [
                defineMetadata(applyDec(targetClass, [classDecs], classDecsHaveThis, targetClass.name, PROP_KIND.CLASS, metadata, initializers), metadata),
