...
export default function rewriteLiveReferences(programPath, metadata, wrapReference) {
            ...
            if (isIdentifier(id) &&
                ...
                for (const localName of Object.keys(decl.getOuterBindingIdentifiers())) {
                    if (exported.has(localName)) {
                        const statement = expressionStatement(
                        // eslint-disable-next-line @typescript-eslint/no-use-before-define
                        buildBindingExportAssignmentExpression(metadata, exported.get(localName), identifier(localName), path.scope));
                        // @ts-expect-error todo(flow->ts): avoid mutations
                        statement._blockHoist = path.node._blockHoist;
                        requeueInParent(path.insertAfter(statement)[0]);
                    }
...
const buildBindingExportAssignmentExpression = (metadata, exportNames, localExpr, scope) => {
    ...
    return (exportNames || []).reduce((expr, exportName) => {
        ...
        return assignmentExpression("=", memberExpression(identifier(exportsObjectName), computed ? stringLiteral(exportName) : identifier(exportName), 
...
const rewriteReferencesVisitor = {
    ReferencedIdentifier(path) {
        ...
        if (importData) {
            if (isInType(path)) {
                throw path.buildCodeFrameError(`Cannot transform the imported binding "${localName}" since it's also used in a type annotation. ` +
            ...
            if (exportedNames?.length > 0 || importData) {
                if (importData) {
                    ...
                    path.replaceWith(sequenceExpression([
                        ...
                        buildBindingExportAssignmentExpression(this.metadata, exportedNames, identifier(localName), path.scope),
                        cloneNode(ref),
                    ]));
                }
            }
    ...
    "ForOfStatement|ForInStatement"(path) {
        ...
        if (!isVariableDeclaration(left)) {
            ...
            path
                .get("left")
                .replaceWith(variableDeclaration("let", [
