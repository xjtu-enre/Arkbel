 ...
 */
export default function normalizeModuleAndLoadMetadata(programPath, exportName, { importInterop, initializeReexports = false, getWrapperPayload, esNamespaceOnly = false, filename, }) {
    if (!exportName) {
        exportName = programPath.scope.generateUidIdentifier("exports").name;
    }
    const stringSpecifiers = new Set();
    nameAnonymousExports(programPath);
    const { local, sources, hasExports } = getModuleMetadata(programPath, { initializeReexports, getWrapperPayload }, stringSpecifiers);
