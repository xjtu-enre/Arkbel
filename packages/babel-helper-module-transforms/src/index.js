...
import normalizeModuleAndLoadMetadata, { hasExports, isSideEffectImport, validateImportInteropOption, } from "./normalize-and-load-metadata.ts";
import * as Lazy from "./lazy-modules.ts";
const { booleanLiteral, callExpression, cloneNode, directive, directiveLiteral, expressionStatement, identifier, isIdentifier, memberExpression, stringLiteral, valueToNode, variableDeclaration, variableDeclarator, } = t;
 ...
 */
export function rewriteModuleStatementsAndPrepareHeader(path, { exportName, strict, allowTopLevelThis, strictMode, noInterop, importInterop = noInterop ? "none" : "babel", 
// TODO(Babel 8): After that `lazy` implementation is moved to the CJS
// transform, remove this parameter.
lazy, getWrapperPayload = Lazy.toGetWrapperPayload(lazy ?? false), wrapReference = Lazy.wrapReference, esNamespaceOnly, filename, constantReexports = process.env.BABEL_8_BREAKING
    ? undefined
    : arguments[1].loose, enumerableModuleMeta = process.env.BABEL_8_BREAKING
    ? undefined
    : arguments[1].loose, noIncompleteNsImportDetection, }) {
...
export function buildNamespaceInitStatements(metadata, sourceMetadata, constantReexports = false, wrapReference = Lazy.wrapReference) {
    ...
    for (const exportName of sourceMetadata.reexportNamespace) {
        ...
        statements.push((!t.isIdentifier(srcNamespace)
            ? template.statement `
