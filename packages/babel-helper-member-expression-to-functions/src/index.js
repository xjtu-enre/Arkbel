import { LOGICAL_OPERATORS, arrowFunctionExpression, assignmentExpression, binaryExpression, booleanLiteral, callExpression, cloneNode, conditionalExpression, identifier, isMemberExpression, isOptionalCallExpression, isOptionalMemberExpression, isUpdateExpression, logicalExpression, memberExpression, nullLiteral, optionalCallExpression, optionalMemberExpression, sequenceExpression, updateExpression, } from "@babel/types";
...
const handle = {
    ...
    handle(member, noDocumentAll) {
        ...
        if (member.isOptionalMemberExpression()) {
            ...
            else if ((process.env.BABEL_8_BREAKING || this.delete) &&
            ...
            const baseMemoised = baseNeedsMemoised
                ...
                cloneNode(baseRef), cloneNode(startingNode))
                : cloneNode(baseRef);
            if (willEndPathCastToBoolean) {
                ...
                replacementPath.replaceWith(conditionalExpression(nullishCheck, isDeleteOperation
                    ? booleanLiteral(true)
        ...
        if ((process.env.BABEL_8_BREAKING || this.delete) &&
            parentPath.isUnaryExpression({ operator: "delete" })) {
            parentPath.replaceWith(this.delete(member));
            return;
        }
