    ...
    return {
        ...
        visitor: {
            ExportNamedDeclaration(path) {
                ...
                if (!t.isExportDefaultSpecifier(specifiers[0]))
                    return;
                const { exported } = specifiers.shift();
                if (specifiers.every(s => t.isExportSpecifier(s))) {
                    specifiers.unshift(t.exportSpecifier(t.identifier("default"), exported));
                    return;
                }
                path.insertBefore(t.exportNamedDeclaration(null, [t.exportSpecifier(t.identifier("default"), exported)], t.cloneNode(source)));
            },
        },
    };
});
