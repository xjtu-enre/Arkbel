...
export default function transpileNamespace(path, allowNamespaces) {
    ...
    if (!allowNamespaces) {
        throw path
            .get("id")
            .buildCodeFrameError("Namespace not marked type-only declare." +
...
function handleVariableDeclaration(node, name, hub) {
    if (node.kind !== "const") {
        throw hub.file.buildCodeFrameError(node, "Namespaces exporting non-const are not supported by Babel." +
...
function handleNested(path, node, parentExport) {
    ...
    const namespaceTopLevel = t.isTSModuleBlock(node.body)
        ? node.body.body
