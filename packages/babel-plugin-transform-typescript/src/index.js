...
import { GLOBAL_TYPES, isGlobalType, registerGlobalType, } from "./global-types.ts";
    ...
    switch (path.parent.type) {
        ...
        case "ExportSpecifier":
            ...
            path.parent.exportKind === "type" ||
                ...
                path.parentPath.parent.exportKind ===
                    "type");
        default:
            return false;
    }
...
export default declare((api, opts) => {
        ...
        constructor(path, classPath) {
            ...
            for (const paramPath of path.get("params")) {
                ...
                if (param.type === "TSParameterProperty") {
                    ...
                    else if (t.isAssignmentPattern(parameter) &&
        ...
        visitor: {
                ...
                enter(path, state) {
                    ...
                    for (let stmt of path.get("body")) {
                        if (stmt.isImportDeclaration()) {
                            ...
                            const isAllSpecifiersElided = () => specifiersLength > 0 &&
            ...
            ExportNamedDeclaration(path, state) {
                ...
                if (path.node.source &&
                    ...
                    path.node.specifiers.every(specifier => specifier.type === "ExportSpecifier" &&
                ...
                if (!path.node.source &&
                    path.node.specifiers.length > 0 &&
                    path.node.specifiers.every(specifier => t.isExportSpecifier(specifier) &&
    ...
    function visitPattern({ node, }) {
        if (node.typeAnnotation)
            node.typeAnnotation = null;
        if (t.isIdentifier(node) && node.optional)
            node.optional = null;
