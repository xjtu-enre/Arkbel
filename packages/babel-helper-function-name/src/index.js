...
import { NOT_LOCAL_BINDING, cloneNode, identifier, isAssignmentExpression, isAssignmentPattern, isFunction, isIdentifier, isLiteral, isNullLiteral, isObjectMethod, isObjectProperty, isRegExpLiteral, isRestElement, isTemplateLiteral, isVariableDeclarator, toBindingIdentifierName, } from "@babel/types";
...
function wrap(state, method, id, scope) {
    if (state.selfReference) {
        if (scope.hasBinding(id.name) && !scope.hasGlobal(id.name)) {
            ...
            const params = template.callee.body
                .body[0].params;
 ...
 */
export default function ({ node, parent, scope, id, }, localBinding = false, supportUnicodeId = false) {
    // has an `id` so we don't need to infer one
    if (node.id)
        return;
    if ((isObjectProperty(parent) || isObjectMethod(parent, { kind: "method" })) &&
