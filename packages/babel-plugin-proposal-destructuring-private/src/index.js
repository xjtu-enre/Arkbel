...
import { hasPrivateKeys, hasPrivateClassElement, transformPrivateKeyDestructuring, buildVariableDeclarationFromParams, } from "./util.ts";
...
export default declare(function ({ assertVersion, assumption, types: t }) {
    assertVersion(process.env.BABEL_8_BREAKING && process.env.IS_PUBLISH
        ? PACKAGE_JSON.version
        : "^7.17.0");
    const { assignmentExpression, assignmentPattern, cloneNode, expressionStatement, isExpressionStatement, isIdentifier, isSequenceExpression, sequenceExpression, variableDeclaration, variableDeclarator, } = t;
    ...
    const privateKeyDestructuringVisitor = {
        Function(path) {
            ...
            const { params: transformedParams, variableDeclaration } = buildVariableDeclarationFromParams(paramsAfterIndex, scope);
        ...
        CatchClause(path) {
            ...
            const { node, scope } = path;
            if (!hasPrivateKeys(node.param))
                return;
            // todo: handle shadowed param as we did in convertFunctionParams
            const ref = scope.generateUidIdentifier("e");
            path
                .get("body")
                .unshiftContainer("body", variableDeclaration("let", [variableDeclarator(node.param, ref)]));
        ...
        ForXStatement(path) {
            ...
            if (leftPath.isVariableDeclaration()) {
                ...
                if (!hasPrivateKeys(left.declarations[0].id))
                    return;
                ...
                if (!hasPrivateKeys(leftPath.node))
                    return;
                // for ({ #x: x } of cls);
                // transforms to:
        ...
        VariableDeclaration(path, state) {
            ...
            for (const declarator of declarations) {
                ...
                declarator.id, declarator.init, scope, 
                /* isAssignment */ false, 
                /* shouldPreserveCompletion */ false, name => state.addHelper(name), objectRestNoSymbols, 
                /* useBuiltIns */ true)) {
        ...
        AssignmentExpression(path, state) {
            ...
            if (!hasPrivateKeys(node.left))
                return;
            const assignments = [];
            const shouldPreserveCompletion = (!isExpressionStatement(parent) && !isSequenceExpression(parent)) ||
                path.isCompletionRecord();
            for (const { left, right } of transformPrivateKeyDestructuring(
            // @ts-expect-error The left of an assignment expression must not be a RestElement
            node.left, node.right, scope, 
            /* isAssignment */ true, shouldPreserveCompletion, name => state.addHelper(name), objectRestNoSymbols, 
            /* useBuiltIns */ true)) {
    ...
    const visitor = {
        Class(path, state) {
            if (!hasPrivateClassElement(path.node.body))
                return;
