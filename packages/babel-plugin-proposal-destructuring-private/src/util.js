...
const { assignmentExpression, binaryExpression, conditionalExpression, cloneNode, isObjectProperty, isPrivateName, memberExpression, numericLiteral, objectPattern, restElement, variableDeclarator, variableDeclaration, unaryExpression, } = types;
 ...
 */
export function buildVariableDeclarationFromParams(params, scope) {
    const { elements, transformed } = buildAssignmentsFromPatternList(params, scope, 
    /* isAssignment */ false);
    return {
        params: elements,
        variableDeclaration: variableDeclaration("var", transformed.map(({ left, right }) => variableDeclarator(left, right))),
    };
}
function buildAssignmentsFromPatternList(elements, scope, isAssignment) {
    const newElements = [], transformed = [];
 ...
 * @param visitor
 */
export function* traversePattern(root, visitor) {
    const stack = [];
    ...
    while ((item = stack.pop()) !== undefined) {
        ...
        switch (node.type) {
            ...
            case "TSNonNullExpression":
                throw new Error(`TypeScript features must first be transformed by ` +
...
export function hasPrivateKeys(pattern) {
    let result = false;
...
export function hasPrivateClassElement(node) {
    return node.body.some(element => isPrivateName(
 ...
 */
export function* privateKeyPathIterator(pattern) {
    const indexPath = [];
 ...
 */
export function* transformPrivateKeyDestructuring(left, right, scope, isAssignment, shouldPreserveCompletion, addHelper, objectRestNoSymbols, useBuiltIns) {
    const stack = [];
    ...
    while ((item = stack.pop()) !== undefined) {
        ...
        if (searchPrivateKey.done) {
            ...
            for (let indexPathIndex = 0, index; (indexPathIndex < indexPath.length &&
                (index = indexPath[indexPathIndex]) !== undefined) ||
                left.type === "AssignmentPattern"; indexPathIndex++) {
                ...
                switch (left.type) {
                    ...
                    case "ArrayPattern": {
                        ...
                        const { elements, transformed } = buildAssignmentsFromPatternList(leftElementsAfterIndex, scope, isAssignment);
