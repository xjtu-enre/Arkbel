...
export default declare(api => {
    ...
    function unwrapArguments({ arguments: args }, scope) {
        const init = [];
        for (let i = 0; i < args.length; i++) {
            ...
            if (!t.isArgumentPlaceholder(node) && !t.isImmutable(node)) {
                ...
                if (t.isSpreadElement(node)) {
                    ...
                    init.push(t.assignmentExpression("=", t.cloneNode(id), 
        ...
        visitor: {
            CallExpression(path) {
                ...
                if (node.callee.type === "MemberExpression") {
                    ...
                    sequenceParts.push(t.assignmentExpression("=", t.cloneNode(receiverLVal), 
                    // @ts-ignore(Babel 7 vs Babel 8) Fixme: support `super.foo(?)`
                    receiver), t.assignmentExpression("=", t.cloneNode(functionLVal), t.memberExpression(t.cloneNode(receiverLVal), property)), ...argsInitializers, t.functionExpression(t.isIdentifier(property)
                        ? t.cloneNode(property)
                        : path.scope.generateUidIdentifierBasedOnNode(property), placeholdersParams, t.blockStatement([
                        t.returnStatement(t.callExpression(t.memberExpression(t.cloneNode(functionLVal), t.identifier("call")), [t.cloneNode(receiverLVal), ...args])),
                    ], []), false, false));
                }
                else {
                    sequenceParts.push(t.assignmentExpression("=", t.cloneNode(functionLVal), 
                    // @ts-expect-error V8 intrinsics will not support partial application
                    node.callee), ...argsInitializers, t.functionExpression(t.isIdentifier(node.callee)
                        ? t.cloneNode(node.callee)
                        : path.scope.generateUidIdentifierBasedOnNode(node.callee), placeholdersParams, t.blockStatement([
                        t.returnStatement(t.callExpression(t.cloneNode(functionLVal), args)),
                    ], []), false, false));
                }
