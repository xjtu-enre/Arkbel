import { isArrayTypeAnnotation, isArrowFunctionExpression, isBinaryExpression, isCallExpression, isExportDeclaration, isForOfStatement, isIndexedAccessType, isMemberExpression, isObjectPattern, isOptionalMemberExpression, isYieldExpression, } from "@babel/types";
...
var CheckParam;
(function (CheckParam) {
    CheckParam[CheckParam["expressionStatement"] = 1] = "expressionStatement";
    CheckParam[CheckParam["arrowBody"] = 2] = "arrowBody";
    CheckParam[CheckParam["exportDefault"] = 4] = "exportDefault";
    CheckParam[CheckParam["forHead"] = 8] = "forHead";
    CheckParam[CheckParam["forInHead"] = 16] = "forInHead";
    CheckParam[CheckParam["forOfHead"] = 32] = "forOfHead";
...
const isClassExtendsClause = (node, parent) => {
    ...
    return ((parentType === "ClassDeclaration" || parentType === "ClassExpression") &&
...
export function ObjectExpression(node, parent, printStack) {
    return isFirstInContext(printStack, CheckParam.expressionStatement | CheckParam.arrowBody);
}
export function DoExpression(node, parent, printStack) {
    // `async do` can start an expression statement
    return (!node.async && isFirstInContext(printStack, CheckParam.expressionStatement));
}
...
export function ClassExpression(node, parent, printStack) {
    return isFirstInContext(printStack, CheckParam.expressionStatement | CheckParam.exportDefault);
}
...
export function Identifier(node, parent, printStack) {
    ...
    if (node.extra?.parenthesized &&
        ...
        if ((rightType === "FunctionExpression" || rightType === "ClassExpression") &&
    ...
    if (node.name === "let") {
        ...
        return isFirstInContext(printStack, isFollowedByBracket
