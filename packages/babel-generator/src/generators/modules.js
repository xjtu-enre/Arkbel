import { isClassDeclaration, isExportDefaultSpecifier, isExportNamespaceSpecifier, isImportDefaultSpecifier, isImportNamespaceSpecifier, isStatement, } from "@babel/types";
...
export function _printAttributes(node) {
...
`);
    ...
    const useAssertKeyword = importAttributesKeyword === "assert" ||
...
export function ImportExpression(node) {
    this.word("import");
    if (node.phase) {
        this.token(".");
        this.word(node.phase);
