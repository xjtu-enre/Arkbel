...
export default declare((api, opts) => {
    ...
    function visitRestElements(path, visitor) {
        path.traverse({
            Expression(path) {
                ...
                if ((isAssignmentPattern(parent) && key === "right") ||
    ...
    function extractNormalizedKeys(node) {
        ...
        for (const prop of props) {
            if (t.isIdentifier(prop.key) && !prop.computed) {
                ...
                keys.push(t.stringLiteral(String(
    ...
    function removeUnusedExcludedKeys(path) {
        ...
        Object.keys(bindings).forEach(bindingName => {
            ...
            if (path.scope.getBinding(bindingName).references >
    ...
    function createObjectRest(path, file, objRef) {
        ...
        return [
            impureComputedPropertyDeclarators,
            restElement.argument,
            t.callExpression(file.addHelper(`objectWithoutProperties${objectRestNoSymbols ? "Loose" : ""}`), [t.cloneNode(objRef), keyExpression]),
        ];
    }
        ...
        visitor: {
            ...
            Function(path) {
                ...
                const IdentifierHandler = function (path, functionScope) {
                    ...
                    if (path.scope.getBinding(name) === functionScope.getBinding(name) &&
            ...
            VariableDeclarator(path, file) {
                ...
                visitRestElements(path.get("id"), path => {
                    ...
                    shouldStoreRHSInTemporaryVariable(originalPath.node.id) &&
                        ...
                        // insert _foo = foo()
                        originalPath.insertBefore(t.variableDeclarator(initRef, originalPath.node.init));
                        // replace foo() with _foo
                        originalPath.replaceWith(t.variableDeclarator(originalPath.node.id, t.cloneNode(initRef)));
                        return;
                    }
                    ...
                    refPropertyPath.forEach(prop => {
                        const { node } = prop;
                        ref = t.memberExpression(ref, t.cloneNode(node.key), node.computed || t.isLiteral(node.key));
                    });
                    ...
                    insertionPath = insertionPath.insertAfter(t.variableDeclarator(argument, callExpression))[0];
            ...
            AssignmentExpression(path, file) {
                ...
                if (leftPath.isObjectPattern() && hasRestElement(leftPath)) {
                    ...
                    const [impureComputedPropertyDeclarators, argument, callExpression] = createObjectRest(leftPath, file, t.identifier(refName));
                    ...
                    nodes.push(t.expressionStatement(t.assignmentExpression("=", argument, callExpression)));
            ...
            ForXStatement(path) {
                ...
                if (!t.isVariableDeclaration(left)) {
                    ...
                    if (body.body.length === 0 && path.isCompletionRecord()) {
                        body.body.unshift(t.expressionStatement(scope.buildUndefinedNode()));
                    }
                    body.body.unshift(t.expressionStatement(t.assignmentExpression("=", left, t.cloneNode(temp))));
                }
                else {
                    // for (var {a, ...b} of []) {}
                    ...
                    body.body.unshift(t.variableDeclaration(node.left.kind, [
            ...
            ArrayPattern(path) {
                ...
                if (objectPatterns.length > 0) {
                    ...
                    const kind = statementNode.type === "VariableDeclaration"
                        ? statementNode.kind
                        : "var";
                    statementPath.insertAfter(t.variableDeclaration(kind, objectPatterns));
                }
            },
            // var a = { ...b, ...c }
