...
import { featuresKey, FEATURES, enableFeature, runtimeKey, hasFeature, } from "./features.ts";
import { generateRegexpuOptions, canSkipRegexpu, transformFlags, } from "./util.ts";
const versionKey = "@babel/plugin-regexp-features/version";
export function createRegExpFeaturePlugin({ name, feature, options = {}, manipulateOptions = () => { }, }) {
    return {
        name,
        manipulateOptions,
        pre() {
            ...
            if (runtime !== undefined) {
                if (file.has(runtimeKey) &&
                    ...
                    throw new Error(`The 'runtime' option must be the same for ` +
                ...
                feature === "namedCaptureGroups") {
                    if (!runtime || !file.has(runtimeKey))
                        file.set(runtimeKey, runtime);
                }
                else {
                    file.set(runtimeKey, runtime);
