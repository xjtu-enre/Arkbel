...
export default declare(api => {
    api.assertVersion(process.env.BABEL_8_BREAKING && process.env.IS_PUBLISH
        ...
        : 7);
    const isFunctionSent = (node) => t.isIdentifier(node.meta, { name: "function" }) &&
        t.isIdentifier(node.property, { name: "sent" });
    const hasBeenReplaced = (node, sentId) => t.isAssignmentExpression(node) &&
    ...
    return {
        ...
        visitor: {
            MetaProperty(path, state) {
                ...
                fnPath.node.body.body.unshift(t.variableDeclaration("let", [
