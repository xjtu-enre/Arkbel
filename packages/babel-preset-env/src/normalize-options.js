...
import { TopLevelOptions, ModulesOption, UseBuiltInsOption, } from "./options.ts";
...
export const validateModulesOption = (modulesOpt = ModulesOption.auto) => {
    ...
    ModulesOption[modulesOpt.toString()] || modulesOpt === ModulesOption.false, `The 'modules' option must be one of \n` +
...
export const validateUseBuiltInsOption = (builtInsOpt = false) => {
    ...
    UseBuiltInsOption[builtInsOpt.toString()] ||
        builtInsOpt === UseBuiltInsOption.false, `The 'useBuiltIns' option must be either
...
export function normalizeCoreJSOption(corejs, useBuiltIns) {
    ...
    if (useBuiltIns && corejs === undefined) {
        if (process.env.BABEL_8_BREAKING) {
            throw new Error("When using the `useBuiltIns` option you must specify" +
    ...
    if (version) {
        if (useBuiltIns) {
            if (process.env.BABEL_8_BREAKING) {
                if (version.major !== 3) {
                    throw new RangeError("Invalid Option: The version passed to `corejs` is invalid. Currently, " +
                        "only core-js@3 is supported.");
                }
                if (typeof rawVersion !== "string" ||
                    !String(rawVersion).includes(".")) {
                    throw new Error('Invalid Option: The version passed to `corejs` is invalid. Please use string and specify the minor version, such as `"3.33"`.');
                }
            }
            else {
...
export default function normalizeOptions(opts) {
    ...
    return {
        bugfixes: v.validateBooleanOption(TopLevelOptions.bugfixes, opts.bugfixes, process.env.BABEL_8_BREAKING ? true : false),
