...
import { addProposalSyntaxPlugins, removeUnnecessaryItems, removeUnsupportedItems, } from "./filter-items.ts";
import moduleTransformations from "./module-transformations.ts";
import normalizeOptions from "./normalize-options.ts";
import { pluginSyntaxMap, proposalPlugins, proposalSyntaxPlugins, } from "./shipped-proposals.ts";
import { plugins as pluginsList, pluginsBugfixes as pluginsBugfixesList, overlappingPlugins, } from "./plugins-compat-data.ts";
...
function getPluginList(proposals, bugfixes) {
    if (proposals) {
        if (bugfixes)
            return pluginLists.withProposals.withBugfixes;
        else
            return pluginLists.withProposals.withoutBugfixes;
    }
    else {
        if (bugfixes)
            return pluginLists.withoutProposals.withBugfixes;
        else
...
function getSpecialModulesPluginNames(modules, shouldTransformDynamicImport) {
    ...
    if (shouldTransformDynamicImport) {
        if (modules && modules !== "umd") {
            ...
            console.warn("Dynamic import can only be transformed when transforming ES" +
            ...
            if (corejs) {
                if (process.env.BABEL_8_BREAKING) {
                    ...
                    if (useBuiltIns === "usage") {
                        if (corejs.major === 2) {
                            polyfillPlugins.push([babel7.pluginCoreJS2, pluginOptions], [babel7.legacyBabelPolyfillPlugin, { usage: true }]);
                        }
                        else {
                            polyfillPlugins.push([pluginCoreJS3, pluginOptions], [
                        ...
                        if (corejs.major === 2) {
                            polyfillPlugins.push([babel7.legacyBabelPolyfillPlugin, { regenerator }], [babel7.pluginCoreJS2, pluginOptions]);
                        }
                        else {
                            polyfillPlugins.push([pluginCoreJS3, pluginOptions], [babel7.legacyBabelPolyfillPlugin, { deprecated: true }]);
                            if (!regenerator) {
                                polyfillPlugins.push([
...
export default declarePreset((api, opts) => {
    ...
    if (process.env.BABEL_8_BREAKING && ("loose" in opts || "spec" in opts)) {
        throw new Error("@babel/preset-env: The 'loose' and 'spec' options have been removed, " +
            "and you should configure granular compiler assumptions instead. See " +
            "https://babeljs.io/assumptions for more information.");
    }
    const { bugfixes, configPath, debug, exclude: optionsExclude, forceAllTransforms, ignoreBrowserslistConfig, include: optionsInclude, modules: optionsModules, shippedProposals, targets: optionsTargets, useBuiltIns, corejs: { version: corejs, proposals }, browserslistEnv, } = normalizeOptions(opts);
    if (!process.env.BABEL_8_BREAKING) {
        // eslint-disable-next-line no-var
...
`);
    ...
    const transformTargets = (process.env.BABEL_8_BREAKING
        ? forceAllTransforms
    ...
    const modules = optionsModules === "auto"
    ...
    const pluginNames = filterItems(compatData, include.plugins, exclude.plugins, transformTargets, getSpecialModulesPluginNames(modules, shouldTransformDynamicImport), process.env.BABEL_8_BREAKING || !loose
        ? undefined
        : ["transform-typeof-symbol"], pluginSyntaxMap);
    if (shippedProposals) {
