...
import { willPathCastToBoolean, findOutermostTransparentParent, } from "./util.ts";
...
export function transformOptionalChain(path, { pureGetters, noDocumentAll }, replacementPath, ifNullish, wrapLast) {
    const { scope } = path;
    ...
    for (let i = optionals.length - 1; i >= 0; i--) {
        ...
        if (isCall && t.isIdentifier(chain, { name: "eval" })) {
            ...
            ref = tmpVar;
            check = t.assignmentExpression("=", t.cloneNode(tmpVar), 
    ...
    if (wrapLast)
        result = wrapLast(result);
    const ifNullishBoolean = t.isBooleanLiteral(ifNullish);
    const ifNullishFalse = ifNullishBoolean && ifNullish.value === false;
    const ifNullishVoid = !ifNullishBoolean && t.isUnaryExpression(ifNullish, { operator: "void" });
    const isEvaluationValueIgnored = (t.isExpressionStatement(replacementPath.parent) &&
    ...
    replacementPath.replaceWith(ifNullishBoolean || (ifNullishVoid && isEvaluationValueIgnored)
...
export function transform(path, assumptions) {
    ...
    if (parentPath.isUnaryExpression({ operator: "delete" })) {
        ...
        let wrapLast;
        if (parentPath.isCallExpression({ callee: maybeWrapped.node }) &&
        ...
        transformOptionalChain(path, assumptions, path, willPathCastToBoolean(maybeWrapped)
            ? t.booleanLiteral(false)
            : scope.buildUndefinedNode(), wrapLast);
    }
}
