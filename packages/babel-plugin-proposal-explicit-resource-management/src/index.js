...
var USING_KIND;
(function (USING_KIND) {
    USING_KIND[USING_KIND["NORMAL"] = 0] = "NORMAL";
    USING_KIND[USING_KIND["AWAIT"] = 1] = "AWAIT";
...
export default declare(api => {
    ...
    const transformUsingDeclarationsVisitor = {
        ForOfStatement(path) {
            ...
            left.kind = "const";
            path.ensureBlock();
            path.node.body.body.unshift(t.variableDeclaration("using", [
            ...
            if (state.availableHelper("usingCtx")) {
                ...
                for (const node of path.node.body) {
                    ...
                    for (const decl of node.declarations) {
                        decl.init = t.callExpression(t.memberExpression(t.cloneNode(ctx), isAwaitUsing ? t.identifier("a") : t.identifier("u")), [decl.init]);
                    }
                }
                if (!ctx)
                    return;
                const disposeCall = t.callExpression(t.memberExpression(t.cloneNode(ctx), t.identifier("d")), []);
      ...
      `;
                ...
                let disposeCall = t.callExpression(state.addHelper("dispose"), [t.cloneNode(stackId), t.cloneNode(errorId), t.cloneNode(hasErrorId)]);
                if (needsAwait)
                    disposeCall = t.awaitExpression(disposeCall);
                const replacement = template.statement.ast `
        try {
          var ${stackId} = [];
          ${path.node.body}
    ...
    const transformUsingDeclarationsVisitorSkipFn = traverse.visitors.merge([
            ...
            transformUsingDeclarationsVisitor,
            {
                ...
                Program(path) {
                    ...
                    if (path.node.sourceType !== "module")
                        return;
                    if (!path.node.body.some(isUsingDeclaration))
                        return;
                    ...
                    for (const stmt of path.get("body")) {
                        ...
                        if (stmt.isExportDefaultDeclaration()) {
                            ...
                            innerBlockBody.push(t.variableDeclaration("var", [
                                t.variableDeclarator(varId, declaration),
                            ]));
                            stmt.replaceWith(t.exportNamedDeclaration(null, [
                        ...
                        if (stmt.isExportNamedDeclaration()) {
                            ...
                            if (!node || t.isFunction(node))
                                continue;
                            stmt.replaceWith(t.exportNamedDeclaration(null, Object.keys(t.getOuterBindingIdentifiers(node, false)).map(id => t.exportSpecifier(t.identifier(id), t.identifier(id)))));
                            shouldRemove = false;
                        }
