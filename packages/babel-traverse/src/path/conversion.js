 ...
 */
export function arrowFunctionToExpression({ allowInsertArrow = true, allowInsertArrowWithRest = allowInsertArrow, noNewArrows = process.env.BABEL_8_BREAKING
    ...
    if (!noNewArrows) {
        ...
        nameFunction(this, true) || fn.node, identifier("bind")), [checkBinding ? identifier(checkBinding.name) : thisExpression()]));
...
noNewArrows = true, allowInsertArrow = true, allowInsertArrowWithRest = true) {
    ...
    if (inConstructor && superCalls.length > 0) {
        if (!allowInsertArrow) {
            throw superCalls[0].buildCodeFrameError("When using '@babel/plugin-transform-arrow-functions', " +
        ...
        if (!allowInsertArrowWithRest) {
            ...
            throw superCalls[0].buildCodeFrameError("When using '@babel/plugin-transform-parameters', " +
    ...
    if (superProps.length > 0) {
        if (!allowInsertArrow) {
            throw superProps[0].buildCodeFrameError("When using '@babel/plugin-transform-arrow-functions', " +
...
function standardizeSuperProperty(superProp) {
    if (superProp.parentPath.isAssignmentExpression() &&
        ...
        if (superProp.node.computed) {
            ...
            assignmentPath
                .get("left")
                .replaceWith(memberExpression(object, assignmentExpression("=", tmp, property), true /* computed */));
            assignmentPath
                .get("right")
                .replaceWith(rightExpression(isLogicalAssignment ? "=" : op, memberExpression(object, identifier(tmp.name), true /* computed */), value));
        }
        else {
            // from: super.foo **= 4;
            ...
            assignmentPath
                .get("right")
                .replaceWith(rightExpression(isLogicalAssignment ? "=" : op, memberExpression(object, identifier(property.name)), value));
        }
        ...
        const computedKey = superProp.node.computed
            ...
            : null;
        const parts = [
            assignmentExpression("=", tmp, memberExpression(superProp.node.object, computedKey
                ? assignmentExpression("=", computedKey, superProp.node.property)
                : superProp.node.property, superProp.node.computed)),
            assignmentExpression("=", memberExpression(superProp.node.object, computedKey ? identifier(computedKey.name) : superProp.node.property, superProp.node.computed), binaryExpression(
            // map `++` to `+`, and `--` to `-`
...
function getSuperPropBinding(thisEnvFn, isAssignment, propName) {
    ...
    return getBinding(thisEnvFn, `superprop_${op}:${propName || ""}`, () => {
        ...
        if (propName) {
            ...
            fnBody = memberExpression(_super(), identifier(method.name), true /* computed */);
        }
        if (isAssignment) {
...
const getScopeInformationVisitor = mergeVisitors([
    {
        ...
        JSXIdentifier(child, { thisPaths }) {
            if (child.node.name !== "this")
                return;
            if (!child.parentPath.isJSXMemberExpression({ object: child.node }) &&
