...
function pushTask(taskName, taskDir, suite, suiteName) {
    ...
    if (!test.validateLogs && (test.stdout.code || test.stderr.code)) {
        throw new Error("stdout.txt and stderr.txt are only allowed when the 'validateLogs' option is enabled: " +
    ...
    if (test.ignoreOutput) {
        ...
        if (!test.validateLogs) {
            throw new Error("ignoreOutput can only be used when validateLogs is true: " +
                taskOptsLoc);
        }
    }
    // Delete to avoid option validation error
...
export function resolveOptionPluginOrPreset(options, optionsDir) {
    ...
    if (options.env) {
        for (const envName in options.env) {
            if (!{}.hasOwnProperty.call(options.env, envName))
                continue;
    ...
    if (options.presets) {
        options.presets = wrapPackagesArray("preset", options.presets, optionsDir).map(function (val) {
            if (val.length > 3) {
                throw new Error("Unexpected extra options " +
