...
export function validateUsage(path, state, tdzEnabled) {
    ...
    for (const name of Object.keys(path.getBindingIdentifiers())) {
        ...
        if (!binding)
            continue;
        if (tdzEnabled) {
            if (injectTDZChecks(binding, state))
    ...
    for (const violation of binding.constantViolations) {
        ...
        if (violation.isAssignmentExpression()) {
            ...
            if (operator === "=") {
                ...
                violation.replaceWith(t.logicalExpression(
                // @ts-expect-error todo: give a better type to operator
                operator.slice(0, -1), left, t.sequenceExpression([right, throwNode])));
            }
            else {
                violation.replaceWith(t.sequenceExpression([
                    t.binaryExpression(
                    // @ts-expect-error todo: give a better type to operator
                    operator.slice(0, -1), left, right),
                    throwNode,
                ]));
            }
        }
            ...
            violation
                .get("left")
                .replaceWith(t.variableDeclaration("var", [
