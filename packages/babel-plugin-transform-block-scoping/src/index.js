...
import { getLoopBodyBindings, getUsageInBody, isVarInLoopHead, wrapLoopBody, } from "./loop.ts";
                ...
                Loop(path, state) {
                    ...
                    const markNeedsBodyWrap = () => {
                        if (throwIfClosureRequired) {
                            throw path.buildCodeFrameError("Compiling let/const in this block would add a closure " +
                    ...
                    if (headPath && isBlockScoped(headPath.node)) {
                        ...
                        for (let name of names) {
                            if (bodyScope?.hasOwnBinding(name))
                                continue; // shadowed
                            ...
                            if (headScope.parent.hasBinding(name) ||
...
function isInLoop(path) {
    if (!path.parentPath)
        return false;
    if (path.parentPath.isLoop())
        return true;
    if (path.parentPath.isFunctionParent())
        return false;
