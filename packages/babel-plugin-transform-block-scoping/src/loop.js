...
const collectCompletionsAndVarsVisitor = {
    ...
    "BreakStatement|ContinueStatement"(path, state) {
        ...
        if (label) {
            if (state.labelsStack.includes(label.name))
                return;
        }
    ...
    VariableDeclaration(path, state) {
        if (path.parent === state.loopNode && isVarInLoopHead(path))
            return;
        if (path.node.kind === "var")
            state.vars.push(path);
    },
};
export function wrapLoopBody(loopPath, captured, updatedBindingsUsages) {
    ...
    for (const [name, updatedUsage] of updatedBindingsUsages) {
        ...
        for (const path of updatedUsage)
            path.replaceWith(t.identifier(innerName));
    }
    ...
    const updaterNode = updater.length > 0
        ? t.expressionStatement(t.sequenceExpression(updater))
        : null;
    if (updaterNode)
        fn.body.body.push(updaterNode);
    ...
    for (const varPath of state.vars) {
        ...
        if (assign.length > 0) {
            ...
            if (!t.isForStatement(varPath.parent, { init: varPath.node }) &&
    ...
    if (labelNum + returnNum === 0) {
        ...
        for (const path of state.breaksContinues) {
            ...
            if (updaterNode)
                path.insertBefore(t.cloneNode(updaterNode));
      ...
      `);
        ...
        if (varPath.isVariableDeclaration()) {
            ...
            bodyStmts.push(t.variableDeclaration("var", [
