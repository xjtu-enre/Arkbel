    ...
    const exprs = decoratedProps.reduce(function (acc, node) {
        ...
        if (t.isClassProperty(node, { static: false })) {
            ...
            const initializer = node.value
                ...
                : t.nullLiteral();
            node.value = t.callExpression(state.addHelper("initializerWarningHelper"), [descriptor, t.thisExpression()]);
            WARNING_CALLS.add(node.value);
            acc.push(t.assignmentExpression("=", t.cloneNode(descriptor), t.callExpression(state.addHelper("applyDecoratedDescriptor"), [
                t.cloneNode(target),
            ...
            acc.push(t.callExpression(state.addHelper("applyDecoratedDescriptor"), [
                t.cloneNode(target),
                t.cloneNode(property),
                t.arrayExpression(decorators.map(dec => t.cloneNode(dec.expression))),
...
const visitor = {
        ...
        if (replacement) {
            const [varDeclPath] = path.replaceWithMultiple([
                ...
                t.exportNamedDeclaration(null, [
                    ...
                    t.cloneNode(replacement.declarations[0].id), t.identifier("default")),
                ]),
            ]);
            if (!decl.node.id) {
    ...
    AssignmentExpression(path, state) {
        if (!WARNING_CALLS.has(path.node.right))
            return;
        path.replaceWith(t.callExpression(state.addHelper("initializerDefineProperty"), [
    ...
    CallExpression(path, state) {
        if (path.node.arguments.length !== 3)
            return;
        if (!WARNING_CALLS.has(path.node.arguments[2]))
            return;
