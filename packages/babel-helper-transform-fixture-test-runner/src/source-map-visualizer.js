...
const padStart = "".padStart
    ? Function.call.bind("".padStart)
    : (str, len, pad) => pad.repeat(Math.max(0, len - str.length)) + str;
const padEnd = "".padStart
    ? Function.call.bind("".padEnd)
...
function joinMultiline(left, right, leftLen) {
    ...
    for (let i = 0; i < linesCount; i++) {
        ...
        if (i < leftLines.length)
            res += padEnd(leftLines[i], leftLen, " ");
        else
            res += " ".repeat(leftLen);
...
export default function visualize(output, map) {
    const sourcesLines = new Map(map.sources.map((source, index) => [
        source,
    ...
    for (let i = ranges.length - 1; i >= 0; i--) {
        ...
        if (original.from.column === original.to.column &&
