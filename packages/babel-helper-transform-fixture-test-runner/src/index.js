...
const cachedScripts = new LruCache({ max: 10 });
...
export function runCodeInTestContext(code, opts, context = sharedTestContext) {
    ...
    try {
        if (opts.filename)
            process.chdir(path.dirname(opts.filename));
...
async function run(task) {
    const { actual, expect: expected, exec, options: opts, doNotSetSourceType, optionsDir, validateLogs, ignoreOutput, stdout, stderr, } = task;
    ...
    if (execCode) {
        ...
        ({ result } = await maybeMockConsole(validateLogs, () => babel.transformAsync(execCode, execOpts)));
    ...
    if (!execCode || inputCode) {
        let actualLogs;
        ({ result, actualLogs } = await maybeMockConsole(validateLogs, () => babel.transformAsync(inputCode, getOpts(actual))));
        ...
        if (validateLogs) {
            ...
            validateFile(normalizeOutput(actualLogs.stdout, normalizationOpts), stdout.loc, process.env.BABEL_8_BREAKING
    ...
    if (task.validateSourceMapVisual === true) {
        ...
        try {
            ...
            if (!process.env.OVERWRITE && task.sourceMapVisual.code)
                throw e;
    ...
    if (opts.sourceMaps === true) {
        try {
            ...
            if (!process.env.OVERWRITE && task.sourceMap)
                throw e;
...
function normalizeOutput(code, { normalizePathSeparator = false, normalizePresetEnvDebug = false } = {}) {
    ...
    if (process.platform === "win32") {
        result = result
            ...
            .replace(new RegExp(escapeRegExp(projectRoot.replace(/\\/g, "/")), "g"), cwdSymbol)
...
export default function (fixturesLoc, name, suiteOpts = {}, taskOpts = {}, dynamicOpts) {
    ...
    for (const testSuite of suites) {
        if (suiteOpts.ignoreSuites?.includes(testSuite.title))
            continue;
        describe(name + "/" + testSuite.title, function () {
            for (const task of testSuite.tests) {
                if (suiteOpts.ignoreTasks?.includes(task.title) ||
                    suiteOpts.ignoreTasks?.includes(testSuite.title + "/" + task.title)) {
                    continue;
                }
                const testFn = task.disabled ? it.skip : it;
                testFn(task.title, async function () {
