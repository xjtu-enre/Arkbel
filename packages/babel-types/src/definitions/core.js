...
import { BINARY_OPERATORS, LOGICAL_OPERATORS, ASSIGNMENT_OPERATORS, UNARY_OPERATORS, UPDATE_OPERATORS, } from "../constants/index.ts";
import { defineAliasedType, assertShape, assertOptionalChainStart, assertValueType, assertNodeType, assertNodeOrValueType, assertEach, chain, assertOneOf, validateOptional, } from "./utils.ts";
    ...
    fields: {
        ...
        left: {
            validate: !process.env.BABEL_TYPES_8_BREAKING
                ...
                : assertNodeType("Identifier", "MemberExpression", "OptionalMemberExpression", "ArrayPattern", "ObjectPattern", "TSAsExpression", "TSSatisfiesExpression", "TSTypeAssertion", "TSNonNullExpression"),
        },
        right: {
    ...
    fields: {
        left: {
            validate: !process.env.BABEL_TYPES_8_BREAKING
                ...
                : assertNodeType("VariableDeclaration", "Identifier", "MemberExpression", "ArrayPattern", "ObjectPattern", "TSAsExpression", "TSSatisfiesExpression", "TSTypeAssertion", "TSNonNullExpression"),
        },
        right: {
            validate: assertNodeType("Expression"),
        },
...
defineType("Identifier", {
    ...
    validate(parent, key, node) {
        ...
        if (parentKey === "property") {
            ...
            if (is("ImportSpecifier", parent, { imported: node }))
                return;
        }
        else if (parentKey === "meta") {
            if (is("MetaProperty", parent, { meta: node }))
                return;
        }
        if (
        ...
        value: {
            validate: chain(assertValueType("number"), Object.assign(function (node, key, val) {
                if (1 / val < 0 || !Number.isFinite(val)) {
                    const error = new Error("NumericLiterals must be non-negative finite numbers. " +
...
defineType("RestElement", {
    ...
    fields: {
        ...
        argument: {
            validate: !process.env.BABEL_TYPES_8_BREAKING
                ? assertNodeType("LVal")
                : assertNodeType("Identifier", "ArrayPattern", "ObjectPattern", "MemberExpression", "TSAsExpression", "TSSatisfiesExpression", "TSTypeAssertion", "TSNonNullExpression"),
        },
    },
    validate(parent, key) {
        if (!process.env.BABEL_TYPES_8_BREAKING)
...
defineType("ExportDefaultDeclaration", {
    ...
    fields: {
        declaration: {
            validate: assertNodeType("TSDeclareFunction", "FunctionDeclaration", "ClassDeclaration", "ArkTSStructDeclaration", "Expression"),
        },
...
defineType("Super", process.env.BABEL_8_BREAKING
    ? undefined
    : {
        aliases: ["Expression"],
    });
